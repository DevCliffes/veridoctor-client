...
// Shape we save for every regular field
type LabeledValue = {
  label: string;
  value: unknown;
};
...
function PrescriptionSection({
  ...
  const updateDrug = (id: string, updates: Partial<Drug>) => {
    onChange({
      ...value,
      drugs: value.drugs.map((d) => (d.id === id ? { ...d, ...updates } : d)),
    });
  };
...
export default function FormDetailPage() {
  ...
  // ─── FIX: Save labels alongside values so captured data can be displayed ───
  const handleSaveCapture = async () => {
    if (!appointmentId) return;
    setSaving(true);
    try {
      // Build { fieldId: { label, value } } so the display side always has labels
      const labeledValues: Record<string, LabeledValue> = {};

      form?.sections.forEach((section) => {
        if (section.isPrescription) return; // handled separately below
        section.fields.forEach((field) => {
          labeledValues[field.id] = {
            label: field.label,
      ...
      // Prescription is a structured object — save it under the section id with its label
      const prescriptionSection = form?.sections.find((s) => s.isPrescription);
      if (prescriptionSection) {
        labeledValues[prescriptionSection.id] = {
          label: "Prescription",
      ...
      case "checkbox":
        return (
          <input
            ...
            checked={val === "true"}
            onChange={(e) =>
              handleFieldChange(field.id, String(e.target.checked))
            }
          />
        );
      case "select":
